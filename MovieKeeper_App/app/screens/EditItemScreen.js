...
export default function EditItemScreen({ navigation, route }) {
	const defaultModalMessage =
		"We couldn't find a movie or show with that barcode. Please enter the title to help others find it.";
		...
		<Screen loading={loading}>
			...
			<View
				...
				{media.Poster ? (
					<Image
						style={
							landscape ? styles.movieImageLandscape : styles.movieImagePortrait
						}
					...
					<Image
						style={
							landscape
								? styles.movieImageLandscapeLogo
								: styles.movieImagePortraitLogo
						}
				...
				<View
					...
					<View style={styles.movieDetailsRow}>
						<Text style={styles.minorDetails}>{getMinorDetails()}</Text>

						{media.ScoreRotten || media.imdbRating ? (
							<View style={styles.movieRatingsContainer}>
								{media.ScoreRotten && (
									<Mk_RottenScore score={media.ScoreRotten} />
								)}
								{media.imdbRating && (
									<Mk_ImdbScore
					...
					<View style={styles.bottomContainer}>
						<Mk_RoundButton
							...
							onPress={() =>
								mode === 'edit' ? confirmDeletion() : navigation.pop()
							}
						/>

						{(mode === 'add' || barcode) && (
							<Mk_Button
								style={styles.wrongMovieBtn}
								text={'Wrong result?'}
								icon={'magnify'}
								onPress={() => {
			...
			<Modal visible={showSelectSeasonsModal} animationType={'slide'}>
