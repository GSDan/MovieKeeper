...
export default function App() {
	...
	const allowedOrientation =
		Math.min(dimensions.width, dimensions.height) > 600 ? 'all' : 'portrait';
	...
	const ViewStackNavigator = () => (
		<ViewStack.Navigator
			...
			<ViewStack.Screen name="List" component={LibraryScreen} />
			<ViewStack.Screen
				name="Edit"
				component={EditItemScreen}
				options={{
	...
	if (initializing)
		return (
			<Mk_Screen style={styles.splash}>
				<Image
					style={styles.logo}
					source={require('./app/assets/adaptive-icon.png')}
				/>
			</Mk_Screen>
		);
